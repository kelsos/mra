...
void DataWrapperS::deleteManga(QString mangaTitle)
{
    try
    {
        if(!database.isOpen())
...
void DataWrapperS::updateNewMangaInfo(QString oldTitle, QString newTitle, QString description, QDateTime publicationDate, QString publicationStatus)
{
    try
    {
        if(!database.isOpen())
...
QStringList DataWrapperS::getAllAuthorNames()
{
	QStringList result;
	try
	{
...
QStringList DataWrapperS::getAllPublisherNames()
{
	QStringList result;
	try
	{
